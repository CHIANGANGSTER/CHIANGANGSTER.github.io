...
| `glass.css` | **（Phase 3C 新增）** 公共毛玻璃契约 | `.glass` / `.glass--panel` / `.glass--card` / `.glass--media` / `.glass--subtle` |
| `magic-bento.css` | **（Phase 3E.1 新增）** MagicBento 卡片样式 | `.mb-card` / `.mb-card::after` / `.mb-ripple` |
| `image-modal.css` | **（Phase 3B 新增）** 图片放大/灯箱样式 | 灯箱遮罩、背景毛玻璃、关闭按钮、弹出动画 |
...
| `image-modal.js` | 图片放大/灯箱 | **（Phase 3B）** 接管 `[data-image-modal]`；ESC / 点击背景关闭 |
| `magic-bento.js` | 卡片 hover/glow/tilt/ripple | **（Phase 3E）** `window.__kbMagicBento` 守卫；读 `data-selectors`；接管 `[data-demo-magnet]` |
| `light-rays.js` | WebGL god-rays 背景 | **（Phase 3F）** `window.__kbLightRays` 守卫；无 WebGL 安全退出 |
| `liquid-glass.js` | SVG displacement map | **（Phase 3F）** `window.__kbLiquidGlass` 守卫；依赖 inline `#liquid-glass-filter` |
...
| 模块 | CSS | JS | 接入页 |
|---|---|---|---|
| image-modal | `image-modal.css` | `image-modal.js` | comfy（`data-image-modal`） |
| glass / card | `glass.css` + `card.css` | — | 全 5 页 |
| media / demo-media | `media.css` | — | 全 5 页（comfy 于 Phase 3H 补链） |
| magic-bento | `magic-bento.css` | `magic-bento.js` | css 全 5 页；js 标准 4 页（comfy inline 变体） |
| light-rays | — | `light-rays.js` | view / text / fusion（3） |
| liquid-glass | — | `liquid-glass.js` | view / text / fusion（3） |

### 已知例外（不得复制到新页面）

| 范围 | 状态 |
|---|---|
| comfy MagicBento inline 变体 | 保留（图片补选 + SDF），§3E.3 |
| comfy LightRays inline 变体 | 保留（紧凑格式），§3F |
| comfy LiquidGlass SDF inline 变体 | 保留（非 sine map），§3F |
| multi-model LightRays inline 变体 | 保留（shader 单行压缩），§3F |
| nav 统一 | 暂缓（roadmap §3I，爆炸半径大） |
| 首页 `index.html` | 永久排除 |
| prompts 页 | 暂缓（独立逻辑） |

### 废弃 CSS 清理（Phase 3H）

`workflow-detail.css` 删除 0 引用废弃选择器（全仓 html/js grep 确认零引用）：
- `.workflow-demo-wrap`（已被 `.demo-media` 取代）
- `.workflow-demo-gif`（已被 `.demo-media__video` 取代）
- `.workflow-demo-title`（无引用）

`@keyframes workflow-demo-rise` 保留（`.chapter` 仍引用）。

### 新页面模板

`docs/development/templates/workflow-detail-template.html` —— 新 workflow 详情页起点。含标准 CSS/JS 引用顺序、demo-media / glass-card / image-modal / magic-bento 示例、light-rays / liquid-glass 按需启用说明，及禁止 inline 效果代码清单。
